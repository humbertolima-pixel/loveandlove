...
export default function ContadorVivo({ dataInicio }: { dataInicio: string }) {
  ...
  return (
    <div className="text-center fade-up">
      <p className="font-body text-xs md:text-sm uppercase tracking-[0.2em] text-rose-600/70 mb-3">
        Juntos há
      </p>
...
function Unidade({ valor, label }: { valor: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <span className="font-display text-4xl md:text-6xl text-wine-text tabular-nums">
        {String(valor).padStart(2, "0")}
      </span>
      <span className="font-body text-[0.65rem] md:text-xs text-rose-600/60 uppercase tracking-wide mt-1">
        {label}
      </span>
    </div>
  );
...
function Separador() {
  return (
    <span className="font-display text-3xl md:text-5xl text-rose-400/60 pb-4 md:pb-5">
      :
    </span>
  );
}
